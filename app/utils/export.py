...
def export_time_series_to_csv(time_series: TimeSeries, filepath: Union[str, Path]) -> bool:
    ...
    try:
        ...
        logger.error(f"Failed to export time series to CSV: {e}", exc_info=True)
        return False
...
def export_quotes(quotes: List[Quote], formats: List[str], 
    ...
    for fmt in formats:
        if fmt.lower() == 'json':
            filename = generate_export_filename(filename_prefix, symbols, 'json')
            filepath = output_dir / filename
            if export_to_json(quotes, filepath):
                result['json'] = str(filepath)
                
        elif fmt.lower() == 'csv':
            filename = generate_export_filename(filename_prefix, symbols, 'csv')
            filepath = output_dir / filename
